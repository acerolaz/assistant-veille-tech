...
class FreshNewsIngester:
            ...
            with httpx.Client() as client:
                for topic in topics:
                    ...
                    if data.get("status") != "ok":
                        logger.error(
                            "NewsAPI error for topic '%s': %s", topic, data.get("message")
                        )
                        continue

                    page_articles = data.get("articles", [])
                    for idx, item in enumerate(page_articles):
                        try:
                            article = Article(
                            ...
                            # Apply since filter: skip articles older than the cutoff
                            if since is not None and article.date is not None:
                                article_dt = (
                                    article.date
                                    if article.date.tzinfo
                                    else article.date.replace(tzinfo=timezone.utc)
                                )
                                since_dt = (
                                    since
                                    if since.tzinfo
                    ...
                    logger.info(
                        "FreshNews fetched %d articles for topic '%s'",
                        len(page_articles),
                        topic,
                    )

        except Exception as exc:
            logger.error("FreshNewsIngester.run failed: %s", exc)
    ...
    def parse_xml(
        ...
        """Parse RSS 2.0 or Atom XML from a WebSub push into Article dicts.
        ...
        # Detect feed format: RSS 2.0 uses <item>, Atom uses <entry>
...
def _to_naive_date_dict(d: dict[str, Any]) -> dict[str, Any]:
    """Return a copy of the article dict with the 'date' field normalised to a
    naive UTC ISO string (no timezone suffix).
...
def _extract_item_fields(
    ...
    # Publication date — RSS uses <pubDate> (RFC 2822), Atom uses <published> (ISO 8601)
    ...
    # Content — RSS uses <description> or <content:encoded>, Atom uses <summary>/<content>
    body = (
